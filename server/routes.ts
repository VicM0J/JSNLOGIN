...
export function registerRoutes(app: Express): Server {
  ...
  registerSystemTicketsRoutes(app);
...
function registerSystemTicketsRoutes(app: Express) {
  const router = Router();

  // Crear ticket - todos los usuarios pueden crear
  router.post("/", async (req, res) => {
    if (!req.isAuthenticated()) return res.status(401).json({ message: "Autenticación requerida" });

    try {
      const user = req.user!;
      const { requesterName, requesterArea, ticketType, otherTypeDescription, description, urgency } = req.body;

      // Validaciones
      if (!ticketType || !description?.trim() || !urgency) {
        return res.status(400).json({ message: "Faltan campos requeridos" });
      }

      if (ticketType === "otro" && !otherTypeDescription?.trim()) {
        return res.status(400).json({ message: "Especifica el tipo de solicitud cuando selecciones 'Otro'" });
      }

      // Generar número de ticket consecutivo con formato JSN-TI-MM-AA-CCC
      const counter = await storage.getNextTicketCounter();
      const now = new Date();
      const month = String(now.getMonth() + 1).padStart(2, '0');
      const year = String(now.getFullYear()).slice(-2);
      const ticketNumber = `JSN-TI-${month}-${year}-${String(counter).padStart(3, '0')}`;

      const ticketData = {
        ticketNumber,
        createdBy: user.id,
        requesterName: requesterName || user.name,
        requesterArea: requesterArea || user.area,
        requestDate: new Date().toISOString().split('T')[0],
        ticketType,
        otherTypeDescription: ticketType === "otro" ? otherTypeDescription : null,
      ...
      console.error('Create system ticket error:', error);
  ...
  router.get("/:id", async (req, res) => {
    ...
    try {
      ...
      const ticket = await storage.getSystemTicketById(ticketId);
      if (!ticket) {
      ...
      console.error('Get system ticket error:', error);
      res.status(500).json({ message: "Error al obtener ticket" });
    }
  });

  // Actualizar estado del ticket - solo área de sistemas
  router.patch("/:id/status", async (req, res) => {
    ...
    try {
      ...
      if (user.area !== 'sistemas') {
        return res.status(403).json({ message: "Solo el área de sistemas puede cambiar el estado de los tickets" });
      }
      ...
      if (status === 'finalizada') {
        if (!solution?.trim()) {
          return res.status(400).json({ message: "La solución es requerida para finalizar el ticket" });
        }
      ...
      await storage.notifyTicketStatusChange(updatedTicket, user.id);
  ...
  app.use("/api/system-tickets", router);
}